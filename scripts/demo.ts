...
// mumbai
const BIP_CONTRACT="0x0a705c9B51B4D6b48d59B5D80BB8Cc7151973a47", TOKEN_BOUND_IMP_ADDR="0x32a507b82822c194aB25931bE5d72772aA4F9F3b",
    BIONIC_LAUNCH_ADDR="0x8956E81d76FDdAbF0de54D8Da0d06c2474DeA340", BIONIC_TOKEN_ADDR="0xbf97d8e33eC1e2a8c339E80334041e2809E0b874", 
    USDT_ADDR="0xe583769738b6dd4E7CAF8451050d1948BE717679";
...
const ENTRY_POINT = "0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789",ERC6551_REGISTRY_ADDR="0x02101dfB77FDE026414827Fdc604ddAF224F0921";
// //goerli
...
async function main(level:number) {
   ...
   /*** Add Project */
    //    level++;
   let pid=1,pledgeAmount=1000,tiers=[4,3,2];
   if(level==3){
    level++;
    ...
    let r=await(await fundingContract.connect(owner).add(BIONIC_TOKEN_ADDR,Date.now()/1000|0,pledgeEnding.getTime()/1000|0,1000,10,tokenAllocationStartTime.getTime()/1000|0,10,1e10,tiers)).wait(1);
   ...
   if(level==4){
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        try {
            ...
            console.log(`approved and moved ${res.events[3].args.value} in  ${res.events[3].args.currency} from ${res.events[3].address} to ${res.events[3].args.spender} `);
        } catch (err) {
   ...
   if (level<=5){
    ...
    let userIdx=0;
    for (let i = 0; i < tiers.length-1; i++) { // leave last teir to be setup by smart contract
        tierUsers[i]=[];
    ...
    for (let i = 0; i < tierUsers.length; i++) {
        ...
        let res=await(await fundingContract.connect(owner).addToTier(pid,i,tierUsers[i])).wait();
        console.log(`Added ${res.events[0]?.args.members.length} members to ${res.events[0]?.args.tierId} for pool ${res.events[0]?.args.pid}`)
    }
   }
...
main(5).then(console.log).catch(console.error)

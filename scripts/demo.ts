        ...
        if (level == 3) {
            ...
            for (let i = 0; i < abstractAccounts.length; i++) {
                ...
                if (await (await users[i].getBalance()).lte(balance)) {
                    ...
                    process.stdout.write(`[+${balance.toNumber()} $wei ${await users[i].getAddress()}] `);
                }
                process.stdout.write(`Funded ${abstractAccounts[i].address}\n`);
            }
        }

        await delay(Date.now() - pledgeStarting.getTime());
        /*** Pledge */
        if (level == 4) {
            level++;
...
//step, pid
main(4, 3).then(console.log).catch(console.error);
